...
public final class DancingLinks implements DancingAlgorithm {

    private final DancingArray struct;
    ...
    public static DancingLinks create(SolutionTree solutionTree, DancingArray struct) {
        return new DancingLinks(solutionTree, struct, AlgorithmListener.SILENT);
    }

    public static DancingLinks createVebose(SolutionTree solutionTree, DancingArray struct, TrackingStream stream) {
        return new DancingLinks(solutionTree, struct, new TracingAlgoListener(stream));
    }

    private DancingLinks(SolutionTree solutions, DancingArray struct, AlgorithmListener listener) {
        this.struct = struct;
        this.solutions = solutions;
        this.listener = listener;
        itemFinder = MinimumReachableValueHeuristic.create();
        printer = StructurePrinters.create(struct);
    }
